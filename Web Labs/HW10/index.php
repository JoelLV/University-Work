<html>
    <head>
        <title>HW10 - Joel Lopez Villarreal</title>
    </head>
    <body>
        ...
        <?php
            ...
            print("<table border=1><tr><th>Delete</th><th>Location</th><th>Open Time</th><th>Closing Time</th></tr>");
            while($row = mysqli_fetch_assoc($result)){
                if ($today == $row['dayofweek']){
                    ...
                    print("<tr" . $ontime . ">");
                    print("<td><button type=\"submit\" name=\"Delete\" value=\"" . $row['sessnum'] . "\">Delete</button><td>" . $row['l-name'] . "</td><td>" . $row['begintime'] . "</td><td>" . $row['endtime'] . "</td></tr>");
                }
            }
            print("</table>");
            mysqli_free_result($result);
